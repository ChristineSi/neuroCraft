...
class TextSimplificationModel:
    def simplify_text(self, text):
        ...
        prompt = f"""
        [Simplify the following text as much as possible for dyslexia readers]
        ...
        - Use short and simple sentences (around 70 characters)
        - Be concise; avoid using long, dense paragraphs.
        - Write in simple clear language using every day simple words.
        - Avoid double negatives.
        - Avoid abbreviations
        - Avoid unnecessary jargon.

        Other rules:
        - Keep the original text meaning.
        - Simplify, but do not summarize, keep a similar length as the Original Text.


        These are examples of dislexya friendly texts::
...
#         Simplify {text} to make it easier to read for people affected by Dyslexia.
#         Include only the simplified text in your response.
        # - Avoid using special characters, such as newline characters \"\\n\"

#         Please use the this writing style instructions: use short and simple sentences (around 70 characters per sentence); Use active rather than passive voice;
#         Be concise: avoid using long, dense paragraphs; Write in simple clear language using every day words;
#         Consider using bullet points and numbering rather than continuous prose; Avoid double negatives; Avoid abbreviations where possible: always provide the expanded form when first used;
#         avoid unnecessary jargon.
